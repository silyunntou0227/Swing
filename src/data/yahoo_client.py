...
# fetch_basic_info の並列取得設定
BASIC_INFO_MAX_WORKERS = 8
BASIC_INFO_TIMEOUT = 10
...
class YahooClient:
    ...
    def _flatten_multi_ticker_df(
        ...
        for ticker in tickers:
            ...
            try:
                ...
                logger.debug(f"Yahoo Finance: {code} データ展開失敗: {e}")
                continue
    ...
    def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 最低限 Code, Date, Close が必要
        essential = {"Code", "Date", "Close"}
        if not essential.issubset(set(existing)):
            missing = essential - set(existing)
            logger.warning(f"Yahoo Finance: 必須カラム不足: {missing}")
            return pd.DataFrame()

        df = df[existing]

        # 数値カラムの型検証（文字列が混入していないか）
    ...
    def fetch_basic_info(self, codes: list[str]) -> pd.DataFrame:
        """銘柄の基本情報（PER, PBR, ROE等）を yfinance から並列取得
        ...
        total = len(codes)
        logger.info(f"Yahoo Finance: 基本情報 {total}銘柄を並列取得中...")

        records = []
        with ThreadPoolExecutor(max_workers=BASIC_INFO_MAX_WORKERS) as executor:
            futures = {
                executor.submit(self._fetch_single_basic_info, code): code
                for code in codes
            }
            done_count = 0
            for future in as_completed(futures):
                done_count += 1
                if done_count % 100 == 0:
                    logger.info(f"Yahoo Finance: 基本情報取得中... {done_count}/{total}")
                try:
                    result = future.result(timeout=BASIC_INFO_TIMEOUT)
                    if result:
        ...
        if not df.empty:
            logger.info(f"Yahoo Finance: 基本情報 {len(df)}銘柄取得完了")
        return df

    def _fetch_single_basic_info(self, code: str) -> dict | None:
        """単一銘柄の基本情報を取得（並列実行用）"""
        try:
