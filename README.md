...
| 2+3 | テクニカル+シグナル | ADX≧25、流動性、ゴールデンクロス等 | →50-100 |
| 4 | ニュース・開示 | MBO/TOB除外、業績修正反映 | →30-50 |

### 11ファクタースコアリング (0-100点)

| ファクター | ウェイト | データソース |
|-----------|---------|-------------|
| トレンド（SMA配列） | 20% | yfinance |
| MACD | 14% | yfinance |
| 出来高 | 11% | yfinance |
| ファンダメンタル | 10% | J-Quants / yfinance |
| RSI（14 + Connors RSI(2)） | 10% | yfinance |
| 一目均衡表 | 12% | yfinance |
| ローソク足パターン | 5% | yfinance |
| リスク/リワード | 5% | yfinance |
| ニュース・開示 | 5% | TDnet / EDINET / NewsAPI |
| 需給（信用残） | 3% | Yahoo Finance |
| **セクターローテーション** | **5%** | **JPX銘柄一覧 + マクロ指標** |
...
| マクロ指標 | yfinance (^N225) | — | 市場環境補正・景気局面判定 |
...
├── sector/                 # セクターローテーション分析
│   ├── sector_config.py    # TOPIX-17分類・33業種マッピング・景気サイクル定義
│   └── sector_analyzer.py  # 景気局面推定・セクタースコア算出
├── scoring/                # スコアリング
│   ├── scorer.py           # 11ファクター加重スコアリング
...
TOPIX-17セクター分類と景気サイクル理論に基づき、マクロ環境に応じたセクター別スコア補正を行う。

### 景気サイクル4局面

| 局面 | 市場環境 | 有望セクター |
|------|----------|-------------|
| 金融相場 | 不況後半→回復、金融緩和 | 情報通信（グロース）、不動産、建設 |
| 業績相場 | 好況期、企業業績拡大 | 機械、素材・化学、鉄鋼、輸送用機器 |
| 逆金融相場 | 景気過熱、利上げ局面 | 銀行、エネルギー資源、卸売 |
| 逆業績相場 | 後退→不況、業績悪化 | 食品、医薬品、電力・ガス（ディフェンシブ） |

### TOPIX-17セクタープロファイル

各セクターに以下の特性値を定義:

- **セクタータイプ**: 景気敏感 / ディフェンシブ / 成長 / 金融
- **β値**: 市場全体に対する感応度（景気敏感 > 1.0、ディフェンシブ < 1.0）
- **金利感応度**: 銀行 +0.9（利ザヤ改善）、不動産 -0.7（調達コスト増）等
- **為替感応度**: 輸送用機器 +0.8（円安メリット）、食品 -0.4（輸入コスト増）等
- **有利な景気局面**: セクターローテーション理論に基づく最適局面

### スコア計算ロジック

1. マクロ指標（日経平均トレンド・モメンタム）から景気局面を自動判定
2. 銘柄の33業種 → TOPIX-17セクターに変換
3. セクター特性 × 景気局面の相性でスコア補正（-10〜+10）
4. 11ファクタースコアリングのウェイト5%として統合
...
セクター分析: 輸送用機器 | 業績相場で有利, 景気敏感（業績相場で上昇）
...
- 指数移動平均線 (EMA 4/22) — 日経225最適化パラメータ
- MACD (4/22/3) — Kang(2021)論文に基づく日本市場最適化
- ADX (14) — トレンド確認閾値 25
- 一目均衡表 (9/26/52)
- グランビルの法則
...
### オシレーター系
- RSI (14) — 中長期トレンド確認
- Connors RSI (2) — 短期平均回帰エントリー（学術研究ベース）
- ストキャスティクス (%K14, %D3)
- ダイバージェンス検出（RSI / ストキャスティクス）
...
| テストファイル | テスト数 | 対象 |
|---------------|---------|------|
| `test_indicators.py` | 14 | トレンド、オシレーター、出来高、パターン、波動分析 |
